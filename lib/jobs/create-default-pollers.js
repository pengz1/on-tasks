...
function createDefaultPollersJobFactory(
    ...
    CreateDefaultPollersJob.prototype.createUcsPoller = function(nodeIds, poller){
        return Promise.forEach(nodeIds, function(nodeId) {
            ...
            .then(function(nodeId){
                ...
                .then(function(obmSetting){
                    if(obmSetting){
                        ...
                        throw new Error('Required ucs-obm-service settings are missing.');
                    }
                })
                .then(function() {
                    poller.node = nodeId;
    ...
    CreateDefaultPollersJob.prototype._run = function _run() {
        ...
        Promise.map(self.options.pollers, function (poller) {
            ...
            if (poller.type === 'redfish') {
                ...
                delete poller.type;
                return self.createUcsPoller(self.context.physicalNodeList || [self.nodeId], poller);
            } else {
                var sourceQuery;
                if (poller.type === 'ipmi') {
